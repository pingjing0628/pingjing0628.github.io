---
title: GoLang-Array Type
date: 2019-12-01 22:58:16
tags: 
  - GoLang
  - Array
categories: 
...
### 宣告
* 使用 `[n]type`來宣告一個陣列，其中 `n`是數字，`type` 為型態
```go=
package main
...
* `[n]int` 也是一種型態，在前面加上 `[m]` 就會成為 `[n]int` 的陣列型態。
```go=
package main
...
* 因此，`[2]int` 才能拿來做最外層陣列的元素型態， 而 `[...]int` 不行。
